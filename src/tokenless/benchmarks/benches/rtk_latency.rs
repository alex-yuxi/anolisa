...
use tokenless_bench::metrics::find_rtk_binary;
...
fn bench_rtk(c: &mut Criterion) {
    ...
    // NOTE: Criterion does not support per-iteration timeout. If rtk hangs,
    // the entire bench group will block until the measurement_time limit (30s)
    // is reached. This is a best-effort safeguard; ensure rtk is responsive
    ...
    for (label, cmd) in rtk_command_samples() {
        group.bench_function(format!("rewrite/{label}"), |b| {
            b.iter(|| {
                ...
                // valid here (0/1/2/3 protocol) — we measure latency only.
                Command::new(&bin)
                    .arg("rewrite")
                    .arg(black_box(cmd))
                    .output()
                    .expect("rtk subprocess must spawn")
            });
        });
    }
