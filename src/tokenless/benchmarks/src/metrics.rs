...
//! `tests/compression_rate.rs`), which makes any compression-behaviour change
...
/// crate, or `rtk` on `PATH`. Mirrors the discovery logic used by
/// `tests/rtk_format_compat.rs`. Returns `None` when none is runnable.
            ...
            Ok(child) => {
                ...
                match rx.recv_timeout(Duration::from_secs(5)) {
                    ...
                    Ok(Err(_)) | Err(_) => {
                        ...
                        let status = if rx.recv_timeout(Duration::from_secs(1)).is_ok() {
                            "timeout_killed"
                        } else {
                            "timeout_kill_failed"
                        };
                        other += 1;
                        samples.push(
                            json!({ "label": label, "exit_code": Value::Null, "status": status }),
                        );
                        continue;
                    }
                }
...
pub fn compression_metrics() -> Value {
    ...
    //
    // NOTE: "toon_only" here encodes the RAW input. In benches/pipeline_latency.rs,
...
/// Alias for [`full_report`] — kept for backward compatibility.
pub fn compression_report() -> Value {
    full_report()
}
