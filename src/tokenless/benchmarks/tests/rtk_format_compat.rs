...
fn rewrite_empty_command_is_safe() {
    ...
    // An empty command must not crash rtk; it either reports a protocol code or
    // fails cleanly (non-panic). We only require that the process terminates
    // with a decodable status.
    let bin = find_rtk().unwrap();
    ...
    let code = out.unwrap().status.code().unwrap_or(-1);
    // Any documented protocol code, or a clean generic error, is acceptable.
    assert!(
        PROTOCOL_CODES.contains(&code) || code == -1 || code > 3,
        "empty-command exit code {code} should be a protocol code or a clean error"
    );
}
